...
export default class CustomerView extends Component {
  constructor(props) {
    ...
    this.handleUpdateCustomerFormData = this.handleUpdateCustomerFormData.bind(
      this
    );
  ...
  handlePolicyFormData(formEntry) {
    ...
    if (
      formEntry.target.id === "issued_at" ||
      formEntry.target.id === "expires_at"
    ) {
      console.log(
        formEntry.target.id,
        " .... : ",
        addedpolicy[formEntry.target.id]
      );
  ...
  fetchCustomer = () => {
    ...
    fetch("https://apis.bimapap.co.ke/apis/customers/" + id)
      .then((response) => {
        if (!response.ok) {
  ...
  deleteCustomer = () => {
    ...
    fetch("https://apis.bimapap.co.ke/apis/customers/delete/" + id, {
      method: "DELETE",
    })
      .then((response) =>
        response.json().then((json) => {
  ...
  // send update request to api to update customer on submit form
  updateCustomer = (entry) => {
    ...
    fetch("https://apis.bimapap.co.ke/apis/customers/update/" + id, {
      method: "PUT",
      headers: {
    ...
    if (this.state.showPolicyFormFields) {
      fetch("https://apis.bimapap.co.ke/apis/policies", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <Fragment>
              ...
              <div className="profile-column">
                ...
                <div className="profile-entry">
                  ...
                  <div className="secondary-phone-number">
                    <a
                      href={"tel:" + this.state.customer.secondary_phone_number}
                    >
        ...
        <dialog id="modal" stack="true">
          <form onSubmit={(entry) => this.updateCustomer(entry)}>
            ...
            <input
              onChange={(entry) => this.handleUpdateCustomerFormData(entry)}
              id="first_name"
              value={this.state.updatedcustomer.first_name}
            ...
            <input
              onChange={(entry) => this.handleUpdateCustomerFormData(entry)}
              id="second_name"
              value={this.state.updatedcustomer.second_name}
            ...
            <input
              onChange={(entry) => this.handleUpdateCustomerFormData(entry)}
              id="other_names"
              value={this.state.updatedcustomer.other_names}
            ...
            <input
              ...
              id="phone_number"
              required
              value={this.state.updatedcustomer.phone_number}
            ...
            <input
              ...
              value={this.state.updatedcustomer.secondary_phone_number}
              placeholder="+254712345678"
              type="tel"
            ></input>
            <br />
            ...
            <input
              onChange={(entry) => this.handleUpdateCustomerFormData(entry)}
              id="national_id"
              value={this.state.updatedcustomer.national_id}
            ...
            <input
              onChange={(entry) => this.handleUpdateCustomerFormData(entry)}
              id="email"
              value={this.state.updatedcustomer.email}
            ...
            <input
              onChange={(entry) => this.handleShowPolicyFormFields(entry)}
              id="showPolicyFormFields"
              type="checkbox"
            ></input>
            <br />
